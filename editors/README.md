# Editor Integration for TJS and AsyncJS

This directory contains syntax highlighting definitions for **TJS** (`.tjs`, the typed
language) and **AsyncJS** (`.ajs`, the sandboxed agent subset).
...
### Uninstalling, and working on the grammar

```bash
npx ajs-install-vscode --uninstall   # remove it
npx ajs-install-vscode --force       # replace a symlinked dev install
```

The installer refuses to overwrite a **symlink** unless you pass `--force`. If you have
linked `editors/vscode` into your extensions directory to work on the grammar, a plain
re-run leaves the link alone rather than silently replacing it with a frozen copy — which
is what it used to do, and which presents as "the grammar stopped updating" with no error
to explain it.

The installed directory is named from the extension manifest (`tjs-lang-<version>`), so an
older install under a previous name or version is not overwritten. The installer reports
any it finds and leaves them for you to remove: it will not delete a directory it cannot
prove it created.
...
- Syntax highlighting for `.tjs` files (types-as-examples, `wasm{}` blocks, `test` blocks)
- Syntax highlighting for `.ajs` files
- Embedded highlighting in `ajs`...`` template literals within TypeScript/JavaScript files
- Red squiggly highlighting for syntax AJS forbids (`new`, `class`, `async`, etc.)
...
- `vscode/syntaxes/tjs.tmLanguage.json` - Standalone .tjs file grammar
...
The TextMate grammars are GENERATED by `bun run build:grammars` from
`editors/build-grammars.ts` — edit that, not the JSON.
