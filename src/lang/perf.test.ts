...
describe('TJS Performance', () => {
  describe('Tight loop overhead', () => {
    it('should measure overhead for simple arithmetic', () => {
      ...
      `)
      const unsafeDouble = new Function(`${unsafeResult.code}; return unsafeDouble;`)()
      ...
      console.log(`    TJS:        ${tjsTime.toFixed(2)}ms (${(tjsTime / legacyTime).toFixed(2)}x)`)
      console.log(`    TJS unsafe: ${unsafeTime.toFixed(2)}ms (${(unsafeTime / legacyTime).toFixed(2)}x)`)
    ...
    it('should measure overhead for object manipulation', () => {
      // Legacy JS
      function legacyTransform(x: number, y: number): { sum: number, product: number } {
      ...
      `)
      const tjsTransform = new Function(`${tjsResult.code}; return tjsTransform;`)()
      ...
      `)
      const unsafeTransform = new Function(`${unsafeResult.code}; return unsafeTransform;`)()
    ...
    it('should measure overhead for array operations', () => {
      ...
      `)
      const unsafeSum = new Function(`${unsafeResult.code}; return unsafeSum;`)()
    ...
    it('should measure try-catch overhead amortization', () => {
      ...
      `)
      const tjsIntensive = new Function(`${tjsResult.code}; return tjsIntensive;`)()
      ...
      `)
      const unsafeIntensive = new Function(`${unsafeResult.code}; return unsafeIntensive;`)()
    ...
    it('should measure error path overhead for unsafe', () => {
      ...
      `)
      const unsafeThrow = new Function(`${unsafeResult.code}; return unsafeThrow;`)()
      ...
      console.log(`    Error path:   ${errorTime.toFixed(2)}ms (${(errorTime / successTime).toFixed(2)}x)`)
