...
export function transpileToJS(
  ...
  // The recursive-Type SOLVER is needed only where an example can recurse, and recursion is
  // reachable only through a NAMED reference (a `ref` marker): an example without one is a
  // finite tree. Gating the solver on markers in general inlined ~6 KB into every file with a
  // TS `number` field — 4.4x a two-field interface (0.14.0 final review, M-3).
  const needsSolver = needsKind && code.includes(`${RT_NS}.__k('ref'`)
  ...
  if (needsRuntime) {
    ...
    if (needsType || needsGeneric) {
      ...
      inlineParts.push(
        ...
        `${
          ...
          needsSolver ? '__kKeys(ex)' : 'Object.keys(ex)'
        };return ks.every(k=>${
          needsKind
      ...
      if (needsKind) {
        ...
        inlineParts.push(
          ...
          //
          // Recursive types are checked by ITERATIVE REFINEMENT — the textbook way to compute
          // a greatest fixed point, so cyclic data is accepted when every node satisfies its
          // type. Inside a solve, every nested match is a LOOKUP of a (value, subexample) pair,
          // never a recursive call: a new pair is assumed true and queued; a worklist evaluates
          // each pair's own shape once (nested parts by their current status); a pair that
          // fails marks the pairs that consulted it dirty, and those are re-evaluated only once
          // every never-evaluated pair is done. Statuses only go true -> false. Sound because a
          // pair is marked false only when it fails given children that truly fail, and the
          // pairs left true form a set in which each holds assuming the rest.
          //
          // No recursion into the DATA, so the JS stack never decides a verdict (Node's is
          // ~8x smaller than Bun's). Four earlier designs recursed — overflow, path-only
          // memo, rollback, deferral with episodes — and each passed example tests and
          // failed real input. Only an example that can reach a `ref` or another runtime type
          // is solved at all (`__kRec`, cached per example): any other has finite depth, and
          // the direct matcher is exact and ~5x cheaper for it. This one is checked against a
          // brute-force oracle over wide
          // random systems (example-kinds-oracle.test.ts), with mutation checks. A work
          ...
          // Only `__ex2js` (`.toJSONSchema()`) reads a marker's schema.
          ...(needsExampleSchema
            ? [
    ...
    if (needsKind && (needsType || needsGeneric)) {
      rtExports.push('__k', '__unk')
      if (needsExampleSchema) rtExports.push('__kjs')
    }
