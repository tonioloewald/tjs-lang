 ...
 */
const INLINE_TYPE_ERROR = `function __arrKinds(v){if(!v.length)return'empty array';const k=[];for(const x of v){const t=x===null?'null':Array.isArray(x)?'array':typeof x;if(!k.includes(t))k.push(t);if(k.length===4)break}return'array of '+k.join(' | ')}
