...
const TJSError = Type('TJSError', undefined, { $error: true, message: '', path: '', stack: [''], expected: '', actual: '', cause: null, loc: { start: 0, end: 0 }, errors: [null] })

const SafetyLevel = Union('SafetyLevel', ['none', 'inputs', 'all'])

const TJSConfig = Type('TJSConfig', undefined, { debug: true, safety: null, requireReturnTypes: true, maxStackSize: 0 })
...
function isNativeType(value = any,typeName = '') {
...
function checkType(value = any,expected = '', path = '') {
...
const ParamMeta = Type('ParamMeta', undefined, { type: null, required: true, default: null, loc: { start: 0, end: 0 } })

function validateArgs(args = {},meta = { params: {} }, funcName = '') {
...
const FunctionMeta = Type('FunctionMeta', undefined, { params: {}, returns: { type: null, safe: true }, unsafe: true, safe: true, unsafeReturn: true, safeReturn: true, name: '' })

function shouldValidateInputs(meta = any) {
...
function shouldValidateOutputs(meta = any) {
...
const SafeCapabilities = Type('SafeCapabilities', undefined, { fetch: undefined, console: undefined })

const SafeFunctionOptions = Generic(['TInputs', 'TOutput'], (x, checkTInputs, checkTOutput) => { return typeof x === 'object' && x !== null && 'inputs' in x && checkTInputs(x.inputs) && 'output' in x && checkTOutput(x.output) && 'body' in x && 'timeoutMs' in x && 'fuel' in x && 'capabilities' in x }, 'SafeFunctionOptions')
...
const EvalOptions = Generic(['TOutput'], (x, checkTOutput) => { return typeof x === 'object' && x !== null && 'code' in x && 'context' in x && 'output' in x && checkTOutput(x.output) && 'timeoutMs' in x && 'capabilities' in x }, 'EvalOptions')
...
function installRuntime() {
    ...
    if (g.__tjs) {
        ...
        if (versionsCompatible(TJS_VERSION, existingVersion)) {
            ...
            if (comparison > 0) {
                console.info(`TJS runtime: upgrading ${existingVersion} → ${TJS_VERSION}`);
                g.__tjs = runtime;
            }
            else {
...
const _original_parseVersion = parseVersion
parseVersion = function(version) {
  if (typeof version !== 'string') {
    return { $error: true, message: 'Invalid arguments', path: 'parseVersion' }
  }
  return _original_parseVersion.call(this, version)
}
parseVersion.__tjs = {
  "params": {
    "version": {
