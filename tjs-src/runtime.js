...
const TJSError = Type('TJSError', undefined, {
  $error: true,
  message: '',
  path: '',
  stack: [''],
...
const TJSConfig = Type('TJSConfig', undefined, {
  debug: true,
  safety: null,
...
const ParamMeta = Type('ParamMeta', undefined, {
  type: null,
  required: true,
  default: null,
  loc: { start: 0, end: 0 },
...
const FunctionMeta = Type('FunctionMeta', undefined, {
  params: {},
  returns: { type: null, safe: true },
  unsafe: true,
  safe: true,
  unsafeReturn: true,
  safeReturn: true,
...
const SafeCapabilities = Type('SafeCapabilities', undefined, {
  fetch: undefined,
  console: undefined,
})

const SafeFunctionOptions = Generic(
  ['TInputs', 'TOutput'],
  (x, checkTInputs, checkTOutput) => {
    return (
      typeof x === 'object' &&
      x !== null &&
      'inputs' in x &&
      checkTInputs(x.inputs) &&
      'output' in x &&
      checkTOutput(x.output) &&
      'body' in x &&
      'timeoutMs' in x &&
      'fuel' in x &&
      'capabilities' in x
