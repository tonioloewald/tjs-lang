...
import { FunctionPredicate } from '/Users/tonioloewald/tjs-lang/src/types/Type'
...
describe('FunctionPredicate runtime', () => {
  it('should create a type that accepts functions', () => {
    ...
    expect(Callback.check((x) => String(x))).toBe(true)
...
describe('Generic FunctionPredicate runtime', () => {
  it('should create a factory from type params', () => {
    ...
    expect(Creator.description).toBe('Creator')
    expect(Creator.typeParamNames).toEqual(['T'])
    expect(Creator.__runtimeType).toBe(true)
  })
  ...
  it('should use default type arg when none provided', () => {
    const Creator = FunctionPredicate('Creator', [['T', 'default']], (T) => ({
      params: { x: 0 },
      returns: T,
    }))
  ...
  it('should handle multiple type params', () => {
    const Mapper = FunctionPredicate('Mapper', ['T', ['U', '']], (T, U) => ({
      params: { input: T },
      returns: U,
    }))
