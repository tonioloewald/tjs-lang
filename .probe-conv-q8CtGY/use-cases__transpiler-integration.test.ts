...
import {
  ...
  createAgent,
  getToolDefinitions,
} from '/Users/tonioloewald/tjs-lang/src/transpiler'
...
describe('Transpiler Integration', () => {
  ...
  describe('Real-world patterns', () => {
    it('should handle research agent pattern', async () => {
      const mockPredict = mock(async (prompt) => {
  ...
  describe('Monadic error flow', () => {
    it('should skip subsequent steps when an error occurs', async () => {
      ...
      const vm = new AgentVM({ step1, step2, step3 })
      const ast = ajs(`
        function test() {
          let a = step1({})
          let b = step2({})
    ...
    it('should allow try/catch to recover from errors', async () => {
      ...
      const vm = new AgentVM({ failingStep, recoveryStep })
      const ast = ajs(`
        function test() {
          let result = 'initial'
          try {
    ...
    it('should return error as result when no return statement reached', async () => {
      ...
      const vm = new AgentVM({ failingStep })
      const ast = ajs(`
        function test() {
          let x = failingStep({})
          return { x }
  ...
  describe('implicit return filtering', () => {
    ...
    it('should strip extra top-level properties not in return schema', async () => {
      const { Agent } = await import('/Users/tonioloewald/tjs-lang/src/builder')
