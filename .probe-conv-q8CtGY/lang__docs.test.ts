...
import {
  ...
  prettifyTestBody,
} from '/Users/tonioloewald/tjs-lang/src/lang/docs'
...
`
      ...
      expect(result.items[0].content).toContain('Module Header')
      expect(result.items[1].type).toBe('doc')
      expect(result.items[1].content).toContain('More docs here')
    })
...
`
      ...
      expect(result.items[1].type).toBe('function')
      expect(result.items[2].type).toBe('doc')
      expect(result.items[2].content).toContain('# Second')
      expect(result.items[3].type).toBe('function')
    })
...
`
      ...
      expect(result.items[0].type).toBe('doc')
      expect(result.items[0].content).toContain('Math Utilities')

      expect(result.items[1].type).toBe('function')
      expect(result.items[1].name).toBe('double')

      expect(result.items[2].type).toBe('function')
      expect(result.items[2].name).toBe('triple')

      expect(result.items[3].type).toBe('doc')
      expect(result.items[3].content).toContain('Notes')
  ...
  describe('TypeScript function syntax', () => {
    it('parses TypeScript function with return type', () => {
      ...
      expect(result.items).toHaveLength(1)
      expect(result.items[0].type).toBe('function')
      expect(result.items[0].signature).toBe(
    ...
    it('parses TypeScript function without return type', () => {
      ...
      const result = generateDocs(source)
      expect(result.items).toHaveLength(1)
      expect(result.items[0].signature).toBe('function log(msg: string)')
    })
...
`
      const result = generateDocs(source)
      expect(result.items).toHaveLength(2)
      expect(result.items[0].signature).toContain(': number')
      expect(result.items[1].signature).toContain(': 0')
    })
  })
})
