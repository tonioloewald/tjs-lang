const __stackSize = 64,
  __stack = new Array(__stackSize)
let __stackHead = 0,
  __stackCount = 0
function pushStack(n) {
  if (!n) return
  __stack[__stackHead] = n
  __stackHead = (__stackHead + 1) % __stackSize
  if (__stackCount < __stackSize) __stackCount++
}
function popStack() {
  if (__stackCount > 0) {
    __stackHead = (__stackHead - 1 + __stackSize) % __stackSize
    __stackCount--
  }
}
function getStack() {
  const o = []
  for (let i = 0; i < __stackCount; i++)
    o.push(
      __stack[(__stackHead - __stackCount + i + __stackSize) % __stackSize]
    )
  return o
}
...
describe('Polymorphic functions', () => {
  function runPoly(source, funcName) {
...
`,
  ...
  describe('Polymorphic constructors', () => {
    function runClass(source, returnExpr) {
      const result = tjs('' + source)
      const savedTjs = globalThis.__tjs
      globalThis.__tjs = createRuntime()
      try {
...
`,
      ...
      expect(val.v2.x).toBe(1)
      expect(val.v2.y).toBeCloseTo(0)
    })
  })
})

describe('Local class extensions', () => {
  function runExt(source, returnExpr) {
    const result = tjs(source)
    const savedTjs = globalThis.__tjs
    globalThis.__tjs = createRuntime()
    try {
