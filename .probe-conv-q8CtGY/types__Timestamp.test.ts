...
import { Timestamp } from '/Users/tonioloewald/tjs-lang/src/types/Timestamp'
...
describe('Timestamp', () => {
  ...
  describe('isValid', () => {
    ...
    it('isValidISO still validates the STRING rendering', () => {
      ...
      expect(Timestamp.isValidISO('2024-01-15')).toBe(false)
  ...
  describe('arithmetic', () => {
    ...
    it('addSeconds', () => {
      ...
      expect(Timestamp.second(result)).toBe(15)
      expect(Timestamp.minute(result)).toBe(31)
    })
    it('addMinutes', () => {
      const result = Timestamp.addMinutes(ts, 45)
      expect(Timestamp.minute(result)).toBe(15)
      expect(Timestamp.hour(result)).toBe(15)
    })
    it('addHours', () => {
      const result = Timestamp.addHours(ts, 12)
      expect(Timestamp.hour(result)).toBe(2)
      expect(Timestamp.day(result)).toBe(16)
    })
    it('addDays', () => {
  ...
  describe('extractors', () => {
    ...
    it('month is 1-based', () => {
      expect(Timestamp.month(ts)).toBe(6)
      expect(Timestamp.month(Timestamp.from(2024, 1, 1))).toBe(1)
      expect(Timestamp.month(Timestamp.from(2024, 12, 1))).toBe(12)
    })
    it('day', () => {
  ...
  describe('boundaries', () => {
    ...
    it('endOfMonth', () => {
      ...
      expect(Timestamp.day(result)).toBe(30)
    ...
    it('endOfMonth handles different month lengths', () => {
      ...
      const feb = Timestamp.from(2024, 2, 15)
      expect(Timestamp.day(Timestamp.endOfMonth(feb))).toBe(29)
      const feb2023 = Timestamp.from(2023, 2, 15)
...
describe('Timestamp: the runtime type agrees with the module', () => {
  it('the runtime type accepts what now() returns', async () => {
    const { Timestamp: TimestampType } = await import(
      '/Users/tonioloewald/tjs-lang/src/types/Type'
    )
  ...
  it('the runtime type rejects the ISO rendering — that is TimestampISO', async () => {
    const { Timestamp: TimestampType, TimestampISO } = await import(
      '/Users/tonioloewald/tjs-lang/src/types/Type'
    )
  ...
  it('the exported predicates are the same predicates, not copies', async () => {
    const { isValidTimestamp, isValidISOTimestamp } = await import(
      '/Users/tonioloewald/tjs-lang/src/types/Type'
    )
