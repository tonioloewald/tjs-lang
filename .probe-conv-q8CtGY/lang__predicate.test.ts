...
function ok(src, opts) {
  return verifyPredicate(src, opts).safe
}
ok.__tjs = {
  params: {
    src: {
...
/* line 11 */
function why(src, opts) {
  return verifyPredicate(src, opts).diagnostics.map((d) => d.message)
}
why.__tjs = {
  params: {
    src: {
...
describe('verifyPredicate — driven by the real atom `effects` tag', () => {
  const effectful = effectfulFromAtoms(coreAtoms)
...
describe('compilePredicate — fuel-bounded + global-shadowed (#3)', () => {
  ...
  it('a huge array exhausts fuel via the per-element callback', () => {
    ...
    expect(m.allPos([1, 2, 3])).toBe(true)
