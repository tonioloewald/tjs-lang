...
import { validatePredicateSchema } from '/Users/tonioloewald/tjs-lang/src/lang/predicate-schema'
...
describe('isAnimation — order-free tokens', () => {
  const valid = [
    '3s ease-in 1s infinite alternate slidein',
    'slidein 3s ease-in',
    'infinite alternate 3s slidein',
    '1s linear',
    '200ms',
    'spin 1s cubic-bezier(0.1, 0.7, 1, 0.1) infinite',
    'spin 1s steps(4, end)',
    'var(--motion)',
    'slide 2s, fade 1s ease-out',
  ]
  for (const v of valid)
    it(`accepts ${v}`, () => expect(isAnimation(v)).toBe(true))
  const invalid = ['3s @@@', '', 'slide 2s,', 42, null]
