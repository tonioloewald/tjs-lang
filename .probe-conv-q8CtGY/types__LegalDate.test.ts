...
import { LegalDate } from '/Users/tonioloewald/tjs-lang/src/types/LegalDate'
...
describe('LegalDate', () => {
  ...
  describe('isValid', () => {
    it('accepts valid YYYY-MM-DD dates', () => {
      ...
      expect(LegalDate.isValid('2024-02-29')).toBe(true)
    })
    it('rejects invalid formats', () => {
      expect(LegalDate.isValid('2024-1-15')).toBe(false)
      expect(LegalDate.isValid('24-01-15')).toBe(false)
      expect(LegalDate.isValid('2024/01/15')).toBe(false)
      expect(LegalDate.isValid('2024-01-15T00:00:00Z')).toBe(false)
    })
    it('rejects invalid dates', () => {
      expect(LegalDate.isValid('2024-02-30')).toBe(false)
      expect(LegalDate.isValid('2023-02-29')).toBe(false)
      expect(LegalDate.isValid('2024-13-01')).toBe(false)
    })
    it('rejects non-strings', () => {
      expect(LegalDate.isValid(123)).toBe(false)
      expect(LegalDate.isValid(null)).toBe(false)
    })
  })
  describe('today', () => {
  ...
  describe('from', () => {
    ...
    it('throws on invalid date', () => {
      ...
      expect(() => LegalDate.from(2023, 2, 29)).toThrow()
    })
  })
  describe('parse', () => {
  ...
  describe('extractors', () => {
    ...
    it('month is 1-based', () => {
      expect(LegalDate.month(date)).toBe(6)
      expect(LegalDate.month('2024-01-15')).toBe(1)
      expect(LegalDate.month('2024-12-15')).toBe(12)
    })
    it('day', () => {
    ...
    it('dayOfYear', () => {
      ...
      expect(LegalDate.dayOfYear('2024-12-31')).toBe(366)
      expect(LegalDate.dayOfYear('2023-12-31')).toBe(365)
    })
    it('quarter', () => {
  ...
  describe('utilities', () => {
    it('isLeapYear', () => {
      ...
      expect(LegalDate.isLeapYear(2000)).toBe(true)
      expect(LegalDate.isLeapYear(1900)).toBe(false)
    })
    it('daysInMonth', () => {
      expect(LegalDate.daysInMonth(2024, 1)).toBe(31)
      expect(LegalDate.daysInMonth(2024, 2)).toBe(29)
      expect(LegalDate.daysInMonth(2023, 2)).toBe(28)
  ...
  describe('comparison', () => {
    ...
    it('isBetween', () => {
      ...
      expect(LegalDate.isBetween(date, date, date2)).toBe(true)
      expect(LegalDate.isBetween(date2, date, date2)).toBe(true)
  ...
  describe('boundaries', () => {
    ...
    it('endOfMonth', () => {
      ...
      expect(LegalDate.endOfMonth('2024-02-15')).toBe('2024-02-29')
      expect(LegalDate.endOfMonth('2023-02-15')).toBe('2023-02-28')
    })
    it('startOfQuarter', () => {
      expect(LegalDate.startOfQuarter(date)).toBe('2024-04-01')
      expect(LegalDate.startOfQuarter('2024-01-15')).toBe('2024-01-01')
      expect(LegalDate.startOfQuarter('2024-10-15')).toBe('2024-10-01')
    })
    it('endOfQuarter', () => {
      expect(LegalDate.endOfQuarter(date)).toBe('2024-06-30')
      expect(LegalDate.endOfQuarter('2024-01-15')).toBe('2024-03-31')
    })
    it('startOfYear', () => {
