...
- [~] **#5 Wire into `FunctionPredicate` / `Type`** — predicate bodies authored in this verified-safe substrate; the real consumer.
  ...
  - [ ] **Extend to `Generic` + `FunctionPredicate` predicate bodies.** Generic-Type predicates compose with type params (`predicate(x, T, U){ return T(x[0]) && U(x[1]) }`); pass the params as `knownPredicates` so they verify instead of falling back. `FunctionPredicate` declares a function *shape* (params/returns), not a boolean body — likely no verify step there; confirm.
