...
- [x] **`TjsStandard`** — RESOLVED 2026-08-02, no blockers; abolished in `4cc307e`. It did
      two unrelated jobs, and both were measured. I had proposed splitting it first;
      abolishing the mode dissolved that question, because deleting the directive deletes the
      confusing name — they simply became two rules of the language.

      **ASI protection** changes behavior in exactly ONE situation: a line beginning `(`, `[`
      ...
      Operator continuations, method chains, multi-line ternaries and argument lists are
      untouched, and `;(…)` is identical in both — so Prettier-formatted code cannot be
      affected.

      **Honest truthiness** differs in exactly THREE cases, all boxed primitives:
      `new Boolean(false)`, `new String('')`, `new Number(0)` — JS calls them truthy, TJS does
      not. Every other value agrees (`0`, `NaN`, `''`, `[]`, `{}`, `0n`, `null`). **No escape is
      needed**: nobody wants `if (new Boolean(false))` to enter the branch.

      - [x] Write the ASI detector — warn with a comment at the site (obligation 3). Shipped
            in `4cc307e`.
      - [x] Then abolish the directive. `4cc307e`.

      _Restored 2026-09-26._ Commit `8b305a3` wrote this entry one character per line (~1,100
      lines), and Prettier then rewrote the fragments (`*` became list markers). It sat
      unnoticed for eight weeks and surfaced when the Virta importer turned the fragments into
      tasks named `T`, `0` and `A`. Decoded from that text; the original wording is also in
      `8b305a3`'s commit message.
...
- [x] **`typescript` is imported by `tjs-lang/lang/from-ts` and declared nowhere a consumer
      sees.** DONE 2026-09-26 (0.14.0-rc.1): declared an OPTIONAL peer (`^5`); the publish
      smoke test now installs declared optional peers before typechecking. `dist/tjs-from-ts.js` does `import o from "typescript"`, but `typescript` is only a
