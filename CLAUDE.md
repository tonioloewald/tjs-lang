...
- **Prettier and markdown — one real hazard, one false alarm.**
  ...
  - _Not Prettier's fault:_ a wrapped prose line that **begins** with `+`, `-`, `*` or `1.` is a list item per CommonMark — the marker is swallowed and vanishes from the rendered output. (`JSON-Schema\n  + \`$predicate\``renders as a nested bullet with the`+`gone.) When Prettier rewrites your`+`bullet to`-`, it is _reporting_ that the renderer already ate it, not breaking your prose. This has bitten CHANGELOG.md and TODO.md. Keep the operator off column one; don't silence Prettier over it.
