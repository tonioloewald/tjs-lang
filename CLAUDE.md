...
- **Dictionary defaults**: `(args = {x: 0, y: 0})` = per-member defaults with merge-on-partial (WebIDL-dictionary semantics): `f({x: 5})` → `{x: 5, y: 0}`, recursive, members validated, excess keys stripped (recorder notice), complete payloads returned by identity. Native-only; `dialect: 'js'`/fromTS keep atomic JS defaults. `:` = required (all members required), `=` = defaulted — mixed shapes use separate params. See `docs/dictionary-defaults.md`.
